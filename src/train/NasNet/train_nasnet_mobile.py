...
train_set = prepare_from_tfrecord(
    tfrecord=join(FEATURES_PATH, "Train_NasNet.tfrecords"), batch=BS, train=True
)

test_set = prepare_from_tfrecord(
    tfrecord=join(FEATURES_PATH, "Test_NasNet.tfrecords"), batch=BS, train=False
)
...
# Rebuild top
outputs = FCHeadNetV2.build(base_model=head, len_class=len(CLASS_NAMES), dense_unit=128)
...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(
        join(MODEL_PATH, "best_nasnet_mobile_1.h5"),
        monitor="val_loss",
        mode="min",
        verbose=1,
        save_best_only=True,
