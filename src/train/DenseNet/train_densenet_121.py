...
# construct our model
print("\n[INFO]: Create model")
head = tf.keras.applications.DenseNet121(
...
# Rebuild top
outputs = FCHeadNetV3.build(base_model=head, len_class=len(CLASS_NAMES), dense_unit=128)

# Compile
model = tf.keras.Model(head.input, outputs, name="DenseNet121")
...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(
        join(MODEL_PATH, "best_densenet_121_1.h5"),
        monitor="val_loss",
        mode="min",
        verbose=1,
        save_best_only=True,
